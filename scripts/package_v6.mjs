...
const STAGE = path.join(ROOT, "_v6_stage", "US-Vehicle-Catalog-App-v6");
const OUT = path.join(APP, "exports", "US-Vehicle-Catalog-App-v6.zip");

fs.rmSync(path.join(ROOT, "_v6_stage"), { recursive: true, force: true });
...
const TOP_FILES = [
  ...
  // Version 6 Title Optimizer documentation
  "TITLE_OPTIMIZER_GUIDE.md", "TITLE_TEMPLATE_GUIDE.md", "TITLE_RULES_GUIDE.md",
  "TITLE_ABBREVIATION_GUIDE.md", "TITLE_REVIEW_GUIDE.md",
];
...
const ROOT_FILES = [
  ...
  "validate_title_optimizer_v6.py",
];
...
fs.copyFileSync(path.join(APP, "data", "catalog-v6.db"),
  path.join(STAGE, "data", "catalog-v6.db"));
for (const f of ["app_verification_report_v6.json",
  "app_verification_report_v5_1.json", "app_verification_report.json"]) {
...
// A .gitignore so the extracted folder is a clean, push-ready git repository.
// node_modules and the built frontend are regenerated by setup-windows.bat;
// runtime working files, uploads and sidecars are never committed. The shipped
// catalog-v6.db IS tracked so the app runs immediately after clone.
fs.writeFileSync(path.join(STAGE, ".gitignore"), [
  "# Dependencies and build output (regenerated by setup-windows.bat)",
  "node_modules/",
  "web/dist/",
  "",
  "# Runtime working files - never commit user data",
  "uploads/*",
  "!uploads/.gitkeep",
  "exports/projects/*",
  "!exports/projects/.gitkeep",
  "backups/*",
  "!backups/.gitkeep",
  "",
  "# SQLite sidecars (the .db files themselves are tracked)",
  "*.db-wal",
  "*.db-shm",
  "",
  "# Temporary and staging artifacts",
  "*.part",
  "*.tmp",
  "*.log",
  "bench/",
  ...
  ".env.*",
  "",
  "# The Node version pin is optional; uncomment to ignore editor folders",
  ".vscode/",
  ".idea/",
  "",
].join("\n"), "utf8");
...
execFileSync("powershell", ["-NoProfile", "-Command",
  `Compress-Archive -Path '${path.join(ROOT, "_v6_stage", "*")}' ` +
